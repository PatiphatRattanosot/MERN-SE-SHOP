...
const serviceList = [
  ...
  },
  {
    ...
    description:
      "Explore products & order with case using our Online Ordering n",
          ...
          <div className="grid sm:grid-cols-2 grid-cols-1 gap-8 item-center">
            {myServices.length > 0 &&
              myServices.map((item) => (
                <div
                  key={item.id}
                  className="shadow-lg rounded-md py-5 px-4 text-center space-y-2 text-red cursor-pointer hover:border hover border-indigo-600 transition-all duration-200"
                >
                  <img
                    src={item.image}
                    alt={item.title}
